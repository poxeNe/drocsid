...
export const chkCombatSkillGains = (player: Character, skillUsed: string) => {
    ...
    if (player.equipped.rightHand?.gripType === 1) {
        ...
        throw new Error(drocsay("ERROR ] Could not set grip type for your weapon! (chkCombatSkillGains)", "red"));
    }

    if (skillUsed === "physical") {

        const randInt = lib.random.int(1, 100)
        const playerWeaponSkill = (player.skills[gripType] += 0.1).toFixed(1);

        if (player.skills[gripType] < 20) {

            if (randInt <= 85) {
                player.skills[gripType] = parseFloat(playerWeaponSkill);
                return true;
            }
